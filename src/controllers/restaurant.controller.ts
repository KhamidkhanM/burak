...
restaurantController.processSignup = async (
  ...
  try {
    ...
    const message =
      err instanceof Errors ? err.message : Message.SOMETHING_WENT_WRONG; // pick the right message
    res.send(
      `<script> alert("${message}"); window.location.replace('/admin/signup') </script>`,
    ); // show alert and send back to signup
  }
};
...
restaurantController.processLogin = async (
  ...
  try {
    ...
    ); // show alert and send back to login
  }
};
...
restaurantController.logout = async (req: AdminRequest, res: Response) => {
  try {
    ...
    req.session.destroy(function () {
      // removes all session data (logs the member out)
...
restaurantController.updateChosenUser = async (req: Request, res: Response) => {
  try {
    ...
    if (err instanceof Errors)
      res.status(err.code).json(err); // known error: use its status code
...
restaurantController.checkAuthSession = async (
  ...
  try {
    ...
    if (req.session?.member)
      res.send(`Hi, ${req.session.member.memberNick}`); // logged in: greet by nickname
...
restaurantController.verifyRestaurant = (
  ...
  if (req.session?.member?.memberType === MemberType.RESTAURANT) {
    // only restaurant accounts may pass
